...
from baselines.textual_inversion.personalized import per_img_token_list
...
def get_bert_token_for_string(tokenizer, string, 
    ...
    token = tokenizer(string, padding="max_length", 
                max_length=MAX_NUM_WORDS, return_tensors="pt") #cond_input.input_ids.to(device)
    #--Update token 
...
class EmbeddingManager(nn.Module):
    def __init__(
        ...
        if hasattr(embedder, 'tokenizer'): # using Stable Diffusion's CLIP encoder
            ...
            self.is_clip = False
            get_token_for_string = partial(get_bert_token_for_string, tokenizer)
            get_embedding_for_tkn = embedder.model.embed_tokens
            # with torch.no_grad():
            # get_embedding_for_tkn = nn.Embedding(30522, 1280)
            token_dim = 1280
        # else: # using LDM's BERT encoder
        #     self.is_clip = False
